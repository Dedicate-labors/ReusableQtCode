...
bool NLicenseFileManager::WriteLicenseConfig(QString strSavePath)
{
    QString strLicenseText = QStringLiteral("%1-%2").arg(QDate::currentDate().toString(QStringLiteral("yyyyMMdd"))).arg(License2String(m_license));

    QAESEncryption encryption(QAESEncryption::AES_128, QAESEncryption::ECB, QAESEncryption::ZERO);
    QByteArray hashKey = QCryptographicHash::hash(m_strKey.toUtf8(), QCryptographicHash::Md5);

    QByteArray baEncodeData = encryption.encode(strLicenseText.toUtf8(), hashKey.toUpper());

    QFile file(QStringLiteral("%1/license.info").arg(strSavePath));
    if(file.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text))
    {
        QTextStream ts(&file);
        ts << baEncodeData.toHex();
        return true;
    }
    return false;
}
